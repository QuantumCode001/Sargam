...
const TRENDING_HINDI_SONGS: Track[] = [
  {
    id: 'BSJa1UytM8w',
    title: 'Saiyaara',
    artist: 'Mohit Chauhan & Tarannum Mallik',
    album: 'Ek Tha Tiger',
    coverUrl: '/saiyaara.png',
    duration: 253,
  ...
  },
  {
    ...
    coverUrl: '/ae_dil_hai_mushkil.png',
    duration: 269,
    genre: 'Bollywood / Romantic',
    mood: 'chill',
    youtubeId: 'ekQKl4JyFEQ'
  },
  {
    id: '-bNwqXvMuB8',
    ...
    coverUrl: '/mehndi_laga_ke_rakhna.jpg',
    duration: 287,
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'home' | 'browse' | 'search' | 'library' | 'settings' | 'ai' | 'artist' | 'album' | 'about'>('home');
  ...
  const handlePlayTrackFromSearch = async (track: Track) => {
    // Play the track instantly
    handlePlayTrack(track);

    // Show a loading toast
    showToastNotification('Generating Autoplay Queue...', `Finding songs related to "${track.title}"`);

    try {
      const res = await fetch(`/api/tracks/related/${track.id}?artist=${encodeURIComponent(track.artist)}&genre=${encodeURIComponent(track.genre || '')}`);
      if (res.ok) {
        const data = await res.json();
        const relatedTracks: Track[] = data.tracks || [];
        if (relatedTracks.length > 0) {
          // Update the active play queue
          const newQueue = [track, ...relatedTracks.filter((t) => t.id !== track.id)];
          setQueue(newQueue);
          showToastNotification('Autoplay Playlist Active', `Added related tracks by "${track.artist}" and others.`);
        }
      }
    } catch (err) {
  ...
  const handleNextTrack = () => {
    ...
    if (shuffle) {
      ...
      // Loop back to the start of the playlist/queue
      nextIndex = 0;
    }
          ...
          <AnimatePresence mode="wait">
              ...
              >
                        ...
                        >
                          <div className="relative aspect-square rounded-xl overflow-hidden mb-2">
                            ...
                            <button
                              ...
                              className="absolute top-2 right-2 p-1 bg-black/50 hover:bg-black/80 rounded-full border border-white/10 text-white keep-white z-20"
                            >
                ...
                <div className="space-y-4">
                  ...
                  <div id="songs-horizontal-grid" className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {TRENDING_HINDI_SONGS.map((track) => (
                      <div
                        key={track.id}
                        onContextMenu={(e) => {
                          e.preventDefault();
                      ...
                      >
                        <div className="relative aspect-square rounded-xl overflow-hidden mb-3">
                          ...
                          <button
                            ...
                            className="absolute top-2 right-2 p-1.5 bg-black/50 hover:bg-black/80 rounded-full border border-white/10 text-white keep-white z-20"
                          >
                ...
                <div className="space-y-4 relative group/scroll-container">
                  ...
                  <div className="relative">
                    ...
                    <div 
                      ...
                      className="flex space-x-6 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent scroll-smooth"
                    >
                      {[
            ...
            {activeTab === 'about' && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0 }}
                className="space-y-10 max-w-2xl mx-auto pb-12"
              >
                <div>
                  <h1 className={`text-3xl sm:text-4xl font-black ${settings.darkMode ? 'text-white' : 'text-zinc-900'}`}>About</h1>
                  <p className="text-sm text-zinc-400 mt-1">The creator behind the full-stack Sargam experience.</p>
                </div>

                {/* Elegant Profile Card */}
                <div className={`relative overflow-hidden rounded-3xl border transition-all duration-300 ${
                  settings.darkMode ? 'bg-zinc-900/50 border-white/5 shadow-2xl' : 'bg-white border-zinc-200 shadow-md'
                }`}>
                  {/* Banner backdrop gradient matching active accentColor */}
                  <div className="h-32 bg-gradient-to-r opacity-90 relative" style={{
                    backgroundImage: `linear-gradient(135deg, ${settings.accentColor}dd, #A855F7dd)`
                  }}>
                    {/* Subtle digital code lines visual overlay */}
                    <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.05)_1px,transparent_1px)] bg-[size:100%_4px] opacity-25" />
                  </div>

                  {/* Profile Info Grid */}
                  <div className="px-6 pb-8 pt-20 relative text-center">
                    {/* User Avatar Circle positioned overlapping the banner in center */}
                    <div className="absolute -top-16 left-1/2 -translate-x-1/2 w-32 h-32 rounded-full border-4 shadow-xl overflow-hidden flex items-center justify-center bg-gradient-to-br from-indigo-500 via-rose-500 to-amber-500 p-[2px]"
                      style={{ borderColor: settings.darkMode ? '#18181b' : '#ffffff' }}
                    >
                      <img src="/developer-avatar.jpg" alt="Ranjit Das" className="w-full h-full rounded-full object-cover" />
                    </div>

                    <div className="space-y-6">
                      {/* Name and Designation */}
                      <div>
                        <h2 className={`text-2xl font-black ${settings.darkMode ? 'text-white' : 'text-zinc-900'}`}>Ranjit Das</h2>
                        <p className="text-xs font-extrabold mt-1 tracking-wider uppercase" style={{ color: settings.accentColor }}>
                          Software Developer & Student
                        </p>
                      </div>

                      {/* Bio introduction */}
                      <p className={`text-sm leading-relaxed max-w-md mx-auto ${settings.darkMode ? 'text-zinc-300' : 'text-zinc-655'} font-semibold`}>
                        Hi, I'm Ranjit Das — an Engineering student at the University of Kalyani, passionate about building modern software and crafting digital experiences that make an impact.
                      </p>

                      {/* Contact & Socials Area */}
                      <div className={`border-t ${settings.darkMode ? 'border-white/5' : 'border-black/5'} pt-6 space-y-4`}>
                        <h4 className={`text-xs font-black uppercase tracking-wider text-center ${settings.darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>Connect With Me</h4>
                        
                        <div className="flex flex-wrap justify-center gap-3">
                          {/* Gmail button */}
                          <a 
                            href="mailto:rd687723@gmail.com"
                            className={`flex items-center space-x-3 px-4 py-2.5 rounded-xl border text-xs font-bold transition active:scale-95 ${
                              settings.darkMode 
                                ? 'bg-white/5 border-white/10 hover:bg-white/10 hover:text-white text-zinc-200' 
                                : 'bg-zinc-105 border-zinc-200 hover:bg-zinc-200 text-zinc-800'
                            }`}
                          >
                            <Mail size={14} style={{ color: settings.accentColor }} />
                            <span>rd687723@gmail.com</span>
                          </a>

                          {/* LinkedIn link */}
                          <a 
                            href="https://www.linkedin.com/in/ranjit-das-0a32773a4/"
                            target="_blank"
                          ...
                          >
                            <img src="/linkedin-icon.png" alt="LinkedIn" className="w-4 h-4 object-contain rounded" />
                            <span>LinkedIn</span>
                          </a>

                          {/* X / Twitter link */}
                          <a 
                            href="https://x.com/Mr_Ranjit1"
                            target="_blank"
                            rel="noopener noreferrer"
                            className={`flex items-center space-x-3 px-4 py-2.5 rounded-xl border text-xs font-bold transition active:scale-95 ${
                              settings.darkMode 
                          ...
                          >
                            <img src="/x-icon.png" alt="X (Twitter)" className={`w-4 h-4 object-contain ${settings.darkMode ? 'invert' : ''}`} />
                            <span>X (Twitter)</span>
                          </a>
                        </div>
                      </div>
      ...
      <AnimatePresence>
        {isPlayerMaximized && currentTrack && (
          <ApplePlayer
            ...
            onTrackContextMenu={(track, x, y) => setContextMenu({ track, x, y })}
      ...
      <AnimatePresence>
        {contextMenu && (
          <ContextMenu
            ...
            onRemoveFromHistory={history.some((h) => h.track.id === contextMenu.track.id) ? handleDeleteFromHistory : undefined}
        ...
        {showEditCoverModal && selectedPlaylist && (
          <div className="fixed inset-0 z-[1050] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className={`w-full max-w-md max-h-[90vh] overflow-y-auto rounded-3xl p-6 shadow-2xl relative z-10 border transition-all duration-300 ${
                settings.darkMode 
                  ? 'bg-zinc-900/90 border-white/10 text-white backdrop-blur-2xl' 
                  : 'bg-white border-zinc-200 text-zinc-900 shadow-2xl'
              }`}
              ...
              <div className="mt-4 space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      { name: 'Cozy Beats', url: '/playlist_cover_1.jpg' },
                      { name: 'Aura Orb', url: '/playlist_cover_2.jpg' },
                      { name: 'Cassette Wall', url: '/playlist_cover_3.jpg' },
                      { name: 'Premium Sound', url: '/playlist_cover_4.jpg' },
                      { name: 'Orange Retro', url: '/playlist_cover_5.jpg' },
                      { name: 'Geometric White', url: '/playlist_cover_6.jpg' },
                      { name: 'Black Cassette', url: '/playlist_cover_7.jpg' },
                      { name: 'Neon Wave', url: '/playlist_cover_8.jpg' },
                      { name: 'Golden Rain', url: '/playlist_cover_9.jpg' },
                      { name: 'Sticker Wall', url: '/playlist_cover_10.jpg' },
                      { name: 'Starry Mountains', url: '/playlist_cover_11.jpg' },
                      { name: 'Purple Waves', url: '/playlist_cover_12.jpg' }
                    ].map((preset) => (
                      <button
                        key={preset.name}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    />
                    {customCoverUrl && (customCoverUrl.startsWith('data:') || customCoverUrl.startsWith('blob:')) && (
